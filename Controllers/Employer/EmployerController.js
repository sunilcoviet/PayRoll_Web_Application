...
const employeerCtrl = {
    ...
    login: async (req, res) => {
        try {
          ...
          const access_token = createAccessToken({ id: user._id, username: user.username, email: user.email, role: user.role });
          const refresh_token = createRefreshToken({ id: user._id, username: user.username, email: user.email, role: user.role });
          ...
          await Users.findOneAndUpdate({_id: user._id}, {is_active: true,access_token: access_token }).then((result)=>{
            res.json({
